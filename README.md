...
- Server Side:
  ConnectionsImpl.java - * This class manages the connection with all clients, pairing each client with a connection handler responsible for sending/receiving messages to/from other clients via the server.
                         ...
                         * map 1 - Connection ID -> ConnectionHandler: allows lookup of a client's handler by their ID. 
                           map 2 - Channel -> (Connection ID, Subscription ID): getting a channel's subscribers' Connection IDs and their Subscription ID.
                           map 3 - Connection ID -> (Subscription ID, Channel): getting a client's subscriptions.
